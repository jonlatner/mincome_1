...
# Difference 2 ----
# diff 2: community effect (baseline)
...
# diff 2: community effect (change)
...
# Create column for Diff 3: experimental effect (manitoba) ----

table_diff_3_com_0$term[1] <- "LMP baseline"
table_diff_3_com_0$order <- "b"
table_diff_3_com_1$term[1] <- "LMP study period"
table_diff_3_com_1$order <- "b"
table_diff_3_com_change$term[1] <- "Change in LMP"
table_diff_3_com_change$order <- "b"

table_diff_3_com_0_se <- table_diff_3_com_0 %>%
  select(term,std.error) %>%
  mutate(order = "se") %>%
  rename(estimate = std.error)

table_diff_3_com_1_se <- table_diff_3_com_1 %>%
  select(term,std.error) %>%
  mutate(order = "se") %>%
  rename(estimate = std.error)

table_diff_3_com_change_se <- table_diff_3_com_change %>%
  select(term,std.error) %>%
  mutate(order = "se") %>%
  rename(estimate = std.error)

df_diff_3_com_control <- bind_rows(table_diff_3_com_0, table_diff_3_com_0_se, table_diff_3_com_1, table_diff_3_com_1_se, table_diff_3_com_change, table_diff_3_com_change_se)
df_diff_3_com_control$term <- factor(df_diff_3_com_control$term, levels = c("LMP baseline", "LMP study period", "Change in LMP"))

df_diff_3_com_control <- df_diff_3_com_control %>%
  arrange(term, order) %>%
  select(term, order, estimate, p.value) %>%
  mutate(across(where(is.numeric), round, 5)) %>%
  rename(estimate_diff_3_com = estimate,
         pvalue_diff_3_com = p.value)

rm(list=ls(pattern="table_diff_3_com"))
