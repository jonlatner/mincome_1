...
df_mincome <- df_long %>%
        rename(famno = famnum,
               ...
               twohead_dummy = h4, # Married
               onehead_dummy = h5, # Single
               age_m = h6, # Age (Male)
               age_f = h7, # Age (Female)
        ...
        select(famno, month, 
               onehead_dummy, twohead_dummy, age_m, age_f, numchild, # family vars
               fh_sch, mh_sch, # education (high school dummy)
               first_irf, last_irf,
...
df_mincome <- df_mincome %>%
        ...
        mutate(count = row_number(),
               ...
               ac = na.locf(ac, na.rm = FALSE), # replace missing value with previous non-missing value
               ac = last(ac), # replace all cases with the last value
               ) %>% 
        ungroup()

# Combine site and treatment
# The assignment cell is assigned to the unit at enrollment, 
# The first digit is the treatment plan assigned to the household 
# and is represented below in terms of guarantee level (at time of enrollment) 
# for a base family of Size 4 and normal reduction rate combinations,
...
# outlined above. (set User Manual for more information)
...
# Site treat
# sitetreat 1 "Winnipeg treat"
# sitetreat 2 "Winnipeg contr"
# sitetreat 3 "Dauphin"
# sitetreat 4 "Rural treat"
# sitetreat 5 "Rural contr"
df_mincome <- df_mincome %>%
        mutate(sitetreat = ifelse(site_code == 1 & treat_recode == 1, yes = 1,
...
df_mincome <- df_mincome %>%
        mutate(sitetreat_2 = ifelse(sitetreat == 3, yes = 1,
                                  ifelse(sitetreat == 2 | sitetreat == 5, yes = 0, no = NA)))
